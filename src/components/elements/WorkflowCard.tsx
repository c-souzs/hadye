...
const WorkflowCard = ({ dataWorkflow, indexWorkflow, maxWorkflows }: WorkflowCardProps) => {
    ...
    return (
        <div className={classNames('text-center relative grid grid-cols-1 items-center gap-x-12 gap-y-8 py-6 lg:grid-cols-2 lg:text-left before:absolute before:hidden lg:before:block before:bottom-0 before:h-[255px] before:w-[385px] before:bg-no-repeat before:bg-center',  {'pb-0': lastElement}, classFilteredAlignRow, classFilteredPositionLeft, classFilteredPositionRight, {'opacity-100': showAnimateCard}, {'opacity-0': !showAnimateCard})} ref={refCard}>
            <div className={classNames({'transition-opacity-transform ease-element-up lg:col-start-2 lg:col-end-3 lg:row-start-1 lg:row-end-2': !positionItem}, {'animate-show-left-element': showAnimateCard && positionItem}, {'animate-show-right-element': showAnimateCard && !positionItem})}>
            ...
            <img src={image} alt={title} className={classNames('block w-full max-h-[450px] h-full object-cover rounded relative z-40 transition-opacity-transform ease-element-up', {'animate-show-right-element': showAnimateCard && positionItem}, {'animate-show-left-element': showAnimateCard && !positionItem})}/>
        </div>
    )
}
